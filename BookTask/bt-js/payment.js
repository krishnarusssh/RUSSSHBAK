$(document).ready(function () {
    $("#paytype li").click(function () {
        ...
        if (cwall < $.trim(total)) {
		    ...
		    $('#divwall').addClass( "wallet_disable" );
        }
    });
...
function fncheck() {
    if ($('#hndptype').val() != '0') {
        ...
        if ($('#hndptype').val() == '8') {
            ObjCard.cno = ObjCard.c1+'' + ObjCard.c2+'' + ObjCard.c3+'' + ObjCard.c4;
...
function BookTask(rid, cid, paymode, bggage) {
    $.ajax({
        url: "../BookTask/Handler/BookTask.ashx",
...
function CardDetails(cno) {
    $.ajax({
        ...
        success: function OnComplete(result) {
            ...
            {
                $("#cimg").attr("src", "../BookTask/img/Mastercard.png");
            }
			if (json.brand == 'VISA')
            {
                $("#cimg").attr("src", "../BookTask/img/visa.png");
            }
			if (json.brand == 'MAESTRO')
            {
                $("#cimg").attr("src", "../BookTask/img/Maestro.png");
            }		  
            } catch (e) {
...
function ApplyPromo(rid, cid, promo) {
    $.ajax({
        ...
        url: "../BookTask/Handler/ApplyPromoCode.ashx",
        contentType: "application/text; charset=utf-8",
        data: {
            'rid': rid,
            'cid': cid,
